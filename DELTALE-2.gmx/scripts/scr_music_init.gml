...
For voice acting, rename the .ogg files to .nv and then put them in NV_AudioFiles
in the Included Files section. Then, put them into an audio stream to play them.

This is to reduce RAM usage which would eventually crash the game.
For example: if I wanted to play one of Gale's voice files, I would maybe do..
    
    // va_galeEN_1 is of Gale saying "WHY WOULD I DO THAT!?"
    voice[1] = audio_create_stream("NV_AudioFiles/va_galeEN_1.nv");
    audio_play_sound(voice[1], 0, false);
    
    if (!audio_is_playing(voice[1])) {
        audio_destroy_stream(voice[1]);
    }
...
crossfadeByMicros[1] = 30000;    // 30ms
crossfadeByMicros[2] = 3000;     // 3ms
